...
try {
    $posts = DB::table('posts')
        ->where('posts.type','secret')
        ->join('users','users.uid','=','posts.uid')
        ->select('users.image','users.name','users.surname','users.secret_opt_name','posts.*')
    ...
    foreach ($posts as $i => $post) {
        $posts[$i]->comments = DB::table('comments')
            ...
            ->select('comments.*','users.name','users.surname','users.image','users.secret_opt_name')
            ->get();
